  ...
  <section id="services" class="relative bg-black py-28 overflow-hidden">
    ...
    <div class="max-w-screen-xl mx-auto px-8 md:px-16">
      ...
      <div class="flex items-center gap-6 mb-6">
        <div class="font-mono text-[9px] text-gold/22 tracking-[0.5em] uppercase">Xizmatlar</div>
      ...
      <div class="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
        ...
        <p ref="descRef" class="max-w-xs font-body text-sm text-gold/32 leading-loose opacity-0">
          Fasad o'rnatishdan tortib yakuniy bezakka qadar —
          to'liq jarayonni boshqaramiz.
        </p>
      </div>
      ...
      <div ref="listRef" class="space-y-px opacity-0">
        ...
        >
          <div class="flex items-start gap-6 md:gap-10 py-8 md:py-10 relative">
            <!-- Number -->
            <span class="font-mono text-[9px] text-gold/22 tracking-[0.3em] w-8 flex-shrink-0 pt-2">
              {{ String(i + 1).padStart(2, '0') }}
            </span>

            <!-- Title + desc -->
            <div class="flex-1 min-w-0">
              <div class="flex flex-col md:flex-row md:items-center gap-3 md:gap-8 mb-3">
                <h3
              ...
              <p
                ...
                :class="activeService === i ? 'max-h-20 opacity-100' : 'max-h-0 opacity-0'"
              >
                {{ service.desc }}
              </p>
            </div>
            ...
            <div
              ...
              :class="activeService === i
                ...
                : 'border-gold/10'"
            >
              <span class="font-mono text-xs text-gold/42 transition-transform duration-400"
      ...
      <!-- Bottom CTA -->
      <div ref="ctaRef" class="mt-16 flex flex-col md:flex-row items-start md:items-center justify-between gap-8 opacity-0">
        <div>
          <div class="font-body text-sm text-gold/42 mb-2">Loyihangiz uchun narx hisoblash kerakmi?</div>
          <div class="font-body text-xs text-gold/22">Bepul konsultatsiya — 24 soat ichida javob beramiz</div>
        </div>
        <button class="btn-primary flex-shrink-0" @click="document.querySelector('#contact')?.scrollIntoView({behavior:'smooth'})">
          Narx So'rash
        </button>
      </div>
    </div>
